...
# 동영상은 시작부터 종료될 때까지 프레임을 지속적으로 받아야 하기 때문에 while문을 게속 반복함
while True:
    ret, movie_image = cam.read()

    # 동영상으로부터 프레임(이미지)를 질 받았으면 실햄함
    if ret is True:
        ...
        for face in faces:
            ...
            face_image = cv2.resize(face_image, (w // mosaice_rate, h // mosaice_rate))
